...
class GUPSBenchSystem(System):
    ...
    def __init__(self, num_cores, mem_type, num_chnls, num_updates):
        ...
        # tgens = []
        # self._avail_slack = (128 * 1048576 * self._num_chnls) / 64
        # self._first_avail_address = 0
        # for i in range(self._num_cores):
        #     slack = randint(0, self._avail_slack) * 64
        #     self._first_avail_address += slack
        #     self._avail_slack = (self._avail_slack * 64 - slack) / 64
        #     print('i: ', i, 'first_avail_address: ', self._first_avail_address)
        #     tgens.append(GUPSGen(mem_size = self._table_size,
        #                         start_addr = self._first_avail_address,
        #                         update_limit = num_updates))
        #     self._first_avail_address += self._table_size
        # self.tgens = tgens

        self.tgens = [GUPSGen(mem_size = self._table_size,
                            # start_addr = i * (self._table_size + 64),
                            # start_addr = i * self._table_size + randint(0, min(self._num_chnls, 16)) * 64,
        ...
        for chnl in range(self._num_chnls * self._bpc):
            if self._mem_type != HBM_1000_4H_1x64:
                ...
                if self._mem_type == FGDRAM:
                    ctrl.dram.read_buffer_size = 16
                    ctrl.dram.write_buffer_size = 20
